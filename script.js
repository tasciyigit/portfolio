...
// Internetsiz Mesajlaşma Cihazı C++ kodunu harici dosyadan yükle
function loadWifiCode() {
    const codeEl = document.getElementById('wifi-code');
    if (!codeEl) return;

    fetch('internetsiz_mesajlasma.cpp.txt')
        .then(response => {
            if (!response.ok) throw new Error('Dosya okunamıyor');
            return response.text();
        })
        .then(text => {
            // HTML yorumları / tagleri çalışmasın diye sadece metin olarak yaz
            codeEl.textContent = text;
            // Yeni içerik için kopyalama butonu ekleyelim
            addCopyButtons();
        })
        .catch(() => {
            codeEl.textContent = '// Kod yüklenemedi. "internetsiz_mesajlasma.cpp.txt" dosyasını klasöre koyduğundan emin ol.';
        });
}